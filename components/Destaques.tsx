  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      <div className="group flex overflow-hidden">
        <div className="flex gap-3 md:gap-4 animate-marquee group-hover:[animation-play-state:paused] pl-4 md:pl-16">
          {loop.map((item, i) => (
            <Link
              key={`${item.ferramenta}-${i}`}
              href="/alugueis"
              className="relative flex-shrink-0 w-[128px] md:w-[150px] rounded-lg border border-white/10 bg-black/70 backdrop-blur-sm overflow-hidden transition duration-300 hover:border-amber-500/50 hover:-translate-y-1"
            >
              <div className="relative w-full aspect-[3/2] bg-gradient-to-br from-zinc-900 to-black">
                ...
                />
                <span className="absolute top-1.5 left-1.5 rounded-full bg-black/70 backdrop-blur px-1.5 py-0.5 text-[8px] font-bold text-emerald-400 border border-emerald-400/30">
                  ⚡ Instantâneo
                </span>
              </div>
